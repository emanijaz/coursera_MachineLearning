...
%               regularization.
%
% Notes: X - num_movies  x num_features matrix of movie features     5 x3
...
%
 ...
 Theta_grad = (R .*( X* Theta' - Y))'*X;
   ...
   J=1/2 *sum(sum((R .*( X* Theta' - Y).^2))) + (lambda /2)*(sum(sum(Theta.^2 , 2)))  + (lambda /2)*(sum(sum(X.^2 , 2)));
